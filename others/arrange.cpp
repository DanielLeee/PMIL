...
int _main(int argc, char **argv)
{
	...
	if (src.back() != '\\')
		src.push_back('\\');
	char pos_dir[MAX_PATH], neg_dir[MAX_PATH];
	sprintf(pos_dir, "%spos\\%d", src.c_str(), Parallel::rank);
	sprintf(neg_dir, "%sneg\\%d", src.c_str(), Parallel::rank);
	if (Parallel::rank == 0) {		
	...
	while (fscanf(fin, "%[^\n]\n", buf) == 1) {
		int l = strlen(buf);
		list.push_back(string(buf, l - 2));
		pos.push_back(buf[l - 1] == '1');
	}
