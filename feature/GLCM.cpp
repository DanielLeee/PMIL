...
void ZBGLCM::ComputeFeature(double&FeatureEnergy, double&FeatureEntropy, double&FeatureInertiaQuadrature, double&FeatureCorrelation, double&FeatureLocalCalm, int** pMatrix, int dim)
{
	int i,j;
	double**pdMatrix;
...
glcmFeature ZBGLCM::GLCMFeature(const Mat& gray,int FilterWindowWidth,int dir)
{
		...
		for(j=0; j<rolltimeW; j++)
		{
			for(p=0; p<FilterWindowWidth; p++)			
				for(q=0; q<FilterWindowWidth; q++)				
					...
					arLocalImage[p][q] = gray.at<uchar>(ImageHeight - 1 - (i * FilterWindowWidth + p), j * FilterWindowWidth + q);
